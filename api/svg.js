...
var init_drawWatermark = __esm({
  "shared/packages/utils/drawWatermark.tsx"() {
    ...
    estimateTextWidth = (text) => Array.from(text).reduce((width, character) => {
      ...
      return width + 8.5;
    }, 0) + Math.max(0, text.length - 1) * 0.32;
    drawWatermark = (selection, chartWidth, chartHeight, text = "star-history.com") => {
      const textRight = chartWidth + 10;
      const iconSize = 20;
      const iconGap = 6;
      const iconX = textRight - estimateTextWidth(text) - iconGap - iconSize;
      selection.append("text").style("font-size", "16px").style("fill", "#666666").style("letter-spacing", "0.02em").attr("transform", `translate(${textRight},${chartHeight + 40})`).attr("text-anchor", "end").text(text);
