...
var init_drawWatermark = __esm({
  "shared/packages/utils/drawWatermark.tsx"() {
    ...
    drawWatermark = (selection, chartWidth, chartHeight) => {
      selection.append("text").style("font-size", "16px").style("fill", "#666666").attr("transform", `translate(${chartWidth - 50},${chartHeight + 40})`).attr("text-anchor", "middle").text("star-history.com");
      selection.append("image").attr("transform", `translate(${chartWidth - 135},${chartHeight + 25})`).attr("height", 20).attr("width", 20).attr("href", iconBase64);
    };
  }
});
...
async function handler(req, res) {
  ...
  try {
    ...
    const output = renderSvg2(repoData, { type, theme, transparent, useLogScale, legendPosition, width });
