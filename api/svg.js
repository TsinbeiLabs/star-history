  ...
  "shared/common/utils.tsx"() {
    ((utils2) => {
      ...
      function getDateString(t, format = "yyyy/MM/dd hh:mm:ss") {
        ...
        const formatedString = format.replace("yyyy", String(year)).replace("MM", String(month)).replace("dd", String(date)).replace("hh", String(hours)).replace("mm", String(minutes)).replace("ss", String(seconds));
      ...
      utils2.convertSVGToDataURL = convertSVGToDataURL;
      ...
      utils2.waitImageLoaded = waitImageLoaded;
      ...
      utils2.calcReadingTime = calcReadingTime;
      ...
      utils2.getBase64Image = getBase64Image;
      function absolutifyLink(rel) {
      ...
      utils2.absolutifyLink = absolutifyLink;
    })(utils || (utils = {}));
    utils_default = utils;
  }
});

// shared/common/api.tsx
function getApiUrl(repo, resource, page) {
  if (typeof window === "undefined") {
    if (resource === "repo") {
    ...
    const pageQuery = page === void 0 ? "" : `&page=${page}`;
...
var import_axios, API_PER_PAGE, REQUEST_TIMEOUT_MS, api, api_default;
var init_api = __esm({
  "shared/common/api.tsx"() {
    ...
    ((api2) => {
      async function getRepoStargazers(repo, token, page) {
        return import_axios.default.get(getApiUrl(repo, "stargazers", page), {
          headers: getHeaders(token),
          timeout: REQUEST_TIMEOUT_MS
        });
      }
      api2.getRepoStargazers = getRepoStargazers;
      async function getRepoStargazersCount(repo, token) {
        const { data } = await import_axios.default.get(getApiUrl(repo, "repo"), {
          headers: getHeaders(token),
          timeout: REQUEST_TIMEOUT_MS
        });
        return data.stargazers_count;
      }
      api2.getRepoStargazersCount = getRepoStargazersCount;
      async function getRepoStarRecords(repo, token, maxRequestAmount) {
        ...
        if (pageCount < maxRequestAmount) {
          requestPages.push(...utils_default.range(1, pageCount));
        } else {
          utils_default.range(1, maxRequestAmount).map((i) => {
        ...
        const starRecordsMap = /* @__PURE__ */ new Map();
        if (requestPages.length < maxRequestAmount) {
          ...
          for (let i = 0; i < starRecordsData.length; ) {
            starRecordsMap.set(utils_default.getDateString(starRecordsData[i].starred_at), i + 1);
          ...
          resArray.map(({ data }, index) => {
            if (data.length > 0) {
              ...
              starRecordsMap.set(utils_default.getDateString(starRecord.starred_at), pageStartPosition);
            }
          });
        }
        const starAmount = await getRepoStargazersCount(repo, token);
        starRecordsMap.set(utils_default.getDateString(Date.now()), starAmount);
        const starRecords = [];
      ...
      api2.getRepoStarRecords = getRepoStarRecords;
      async function getRepoLogoUrl(repo, token) {
        const { data } = await import_axios.default.get(getApiUrl(repo, "owner"), {
          headers: getHeaders(token),
          timeout: REQUEST_TIMEOUT_MS
        });
        return data.avatar_url;
      }
      api2.getRepoLogoUrl = getRepoLogoUrl;
    })(api || (api = {}));
    api_default = api;
  }
});
...
__export(chart_exports, {
  DEFAULT_MAX_REQUEST_AMOUNT: () => DEFAULT_MAX_REQUEST_AMOUNT,
  convertDataToChartData: () => convertDataToChartData,
  convertStarDataToChartData: () => convertStarDataToChartData,
  getRepoData: () => getRepoData,
  getReposStarData: () => getReposStarData
});
var DEFAULT_MAX_REQUEST_AMOUNT, STAR_HISTORY_LOGO_URL, getReposStarData, getRepoData, convertStarDataToChartData, convertDataToChartData;
var init_chart = __esm({
  "shared/common/chart.tsx"() {
    init_api();
    init_utils();
    DEFAULT_MAX_REQUEST_AMOUNT = 15;
    STAR_HISTORY_LOGO_URL = "https://avatars.githubusercontent.com/u/124480067";
    getReposStarData = async (repos, token = "", maxRequestAmount = DEFAULT_MAX_REQUEST_AMOUNT) => {
      const repoStarDataCacheMap = /* @__PURE__ */ new Map();
      for (const repo of repos) {
        try {
          const starRecords = await api_default.getRepoStarRecords(repo, token, maxRequestAmount);
    ...
    getRepoData = async (repos, token = "", maxRequestAmount = DEFAULT_MAX_REQUEST_AMOUNT) => {
      const repoDataCacheMap = /* @__PURE__ */ new Map();
      for (const repo of repos) {
        try {
          const [starRecords, logo] = await Promise.all([
            api_default.getRepoStarRecords(repo, token, maxRequestAmount),
            api_default.getRepoLogoUrl(repo, token)
          ]);
            ...
            return [
              {
                ...
                starRecords: [
                  {
                    date: utils_default.getDateString(Date.now(), "yyyy/MM/dd"),
                    count: 0
                  }
                ],
    ...
    convertStarDataToChartData = (reposStarData, chartMode, options) => {
      if (chartMode === "Date") {
        const datasets = reposStarData.map((item) => {
          const { repo, starRecords } = item;
          const chartData = starRecords.map((item2) => {
            return {
              x: new Date(item2.date),
              y: Number(item2.count)
            };
          });
        ...
        const datasets = reposStarData.map((item) => {
          ...
          const chartData = starRecords.map((item2) => {
            return {
              x: utils_default.getTimeStampByDate(new Date(item2.date)) - utils_default.getTimeStampByDate(new Date(started)),
              y: Number(item2.count)
            };
          });
          if (options?.insertZeroPoint && chartData.length > 0 && chartData[0].y > 0) {
            chartData.unshift({
              x: -1,
              // One day before in timeline mode
              y: 0
            });
          }
          return {
    ...
    convertDataToChartData = (repoData, chartMode, options) => {
      if (chartMode === "Date") {
        ...
        const datasets = repoData.map(({ repo, starRecords, logoUrl }) => {
          const chartData = starRecords.map((item) => {
            return {
              x: utils_default.getTimeStampByDate(new Date(item.date)) - utils_default.getTimeStampByDate(new Date(starRecords[0].date)),
              y: Number(item.count)
            };
          });
          if (options?.insertZeroPoint && chartData.length > 0 && chartData[0].y > 0) {
...
// shared/packages/components/ToolTip.tsx
  ...
  "shared/packages/components/ToolTip.tsx"() {
    ToolTip = class {
      ...
      constructor({ selection, title, items, position, backgroundColor, strokeColor }) {
        ...
        this.tipBackground = this.svg.append("rect").style("fill", this.backgroundColor).attr("fill-opacity", 0.9).attr("stroke", strokeColor).attr("stroke-width", 2).attr("rx", 5).attr("ry", 5).attr("filter", this.filter).attr("width", this._getBackgroundWidth()).attr("height", this._getBackgroundHeight()).attr("x", 5).attr("y", 5);
      ...
      _generateTipItem(item, i) {
        ...
        svg.append("text").style("font-size", "15px").style("fill", this.strokeColor).attr("x", 15 + 12).attr("y", 37 + 20 * i + 8).text(item.text);
...
var getNumberFormatUnit, getFormatNumber, getFormatNumber_default;
var init_getFormatNumber = __esm({
  "shared/packages/utils/getFormatNumber.tsx"() {
    getNumberFormatUnit = (n) => {
      if (n >= 1e6) {
        return 1e6;
      }
      if (n >= 300) {
    ...
    getFormatNumber_default = getFormatNumber;
  }
});

// shared/packages/utils/getFormatTimeline.tsx
var import_dayjs, import_duration, import_relativeTime, getTimestampFormatUnit, getFormatTimeline, getFormatTimeline_default;
var init_getFormatTimeline = __esm({
  "shared/packages/utils/getFormatTimeline.tsx"() {
    import_dayjs = __toESM(require("dayjs"), 1);
    import_duration = __toESM(require("dayjs/plugin/duration"), 1);
    import_relativeTime = __toESM(require("dayjs/plugin/relativeTime"), 1);
    import_dayjs.default.extend(import_duration.default);
    import_dayjs.default.extend(import_relativeTime.default);
    getTimestampFormatUnit = (timestamp) => {
      let timelineUnit = "day";
      if (import_dayjs.default.duration(timestamp).asYears() > 1) {
        timelineUnit = "year";
      } else if (import_dayjs.default.duration(timestamp).asMonths() > 1) {
        timelineUnit = "month";
      } else if (import_dayjs.default.duration(timestamp).asWeeks() > 1) {
        timelineUnit = "week";
      }
      return timelineUnit;
    };
    getFormatTimeline = (timestamp, type = "day") => {
      if (timestamp === 0) {
        return "day one";
      }
    ...
    getFormatTimeline_default = getFormatTimeline;
  }
});

// shared/packages/utils/drawAxis.tsx
var import_d3_axis, drawXAxis, drawYAxis;
var init_drawAxis = __esm({
  "shared/packages/utils/drawAxis.tsx"() {
    import_d3_axis = require("d3-axis");
    init_getFormatNumber();
    init_getFormatTimeline();
    drawXAxis = (selection, { xScale, tickCount, moveDown, fontFamily, stroke, type }) => {
      const xAxisGenerator = (0, import_d3_axis.axisBottom)(xScale).tickSize(0).tickPadding(6).ticks(tickCount);
      if (type === "Number") {
        let index = 1;
        let type2 = void 0;
        xAxisGenerator.tickFormat((d) => {
          ...
          if (!type2) {
            type2 = getTimestampFormatUnit(timestamp);
          }
          return getFormatTimeline_default(timestamp, type2);
        });
      }
    ...
    drawYAxis = (selection, { yScale, tickCount, fontFamily, stroke, useLogScale }) => {
      let type = void 0;
      const yAxisGenerator = (0, import_d3_axis.axisLeft)(yScale).tickSize(1).tickPadding(6);
      if (useLogScale) {
        ...
        let tickCount2 = 1;
        const maxTicks = 6;
        while (Math.pow(10, power) <= maxValue && tickCount2 < maxTicks) {
          const tick = Math.pow(10, power);
          logTicks.push(tick);
          tickCount2++;
          power++;
        }
        if (tickCount2 < maxTicks && maxValue > logTicks[logTicks.length - 1]) {
        ...
        yAxisGenerator.tickValues(logTicks).tickFormat((d) => {
          ...
          return getFormatNumber_default(d, type);
        });
      } else {
...
// shared/packages/utils/addFilter.tsx
var addFilter, addFilter_default;
var init_addFilter = __esm({
  "shared/packages/utils/addFilter.tsx"() {
    addFilter = (selection) => {
...
var init_addFont = __esm({
  "shared/packages/utils/addFont.tsx"() {
    ...
    addFont_default = addFont;
  }
});

// shared/packages/utils/drawLabels.tsx
var drawTitle, drawXLabel, drawYLabel;
var init_drawLabels = __esm({
  "shared/packages/utils/drawLabels.tsx"() {
    drawTitle = (selection, text, logoURL, color, chartWidth) => {
      ...
      selection.append("svg").append("defs").append("clipPath").attr("id", "clip-circle-title").append("circle").attr("r", 11).attr("cx", clipX).attr("cy", 12 + 11);
      if (logoURL) {
    ...
    drawXLabel = (selection, text, color) => {
    ...
    drawYLabel = (selection, text, color, offsetY = 6) => {
...
// shared/packages/utils/drawLegend.tsx
var import_uniq, drawLegend, drawLegend_default;
var init_drawLegend = __esm({
  "shared/packages/utils/drawLegend.tsx"() {
    import_uniq = __toESM(require("lodash/uniq"), 1);
    drawLegend = (selection, { items, strokeColor, backgroundColor, legendPosition, chartWidth, chartHeight }) => {
      ...
      let maxTextLength = 0;
      const shouldDrawLogo = (0, import_uniq.default)(items.map((i) => i.text.split("/")[0])).length > 1;
      items.forEach((item) => {
...
var init_drawWatermark = __esm({
  "shared/packages/utils/drawWatermark.tsx"() {
    ...
    drawWatermark = (selection, chartWidth, chartHeight, text = "star-history.com") => {
      ...
      const iconSize = 20;
      const iconGap = 6;
      const watermarkText = selection.append("text").style("font-size", "16px").style("fill", "#666666").style("letter-spacing", "0.02em").attr("transform", `translate(${textRight},${chartHeight + 40})`).attr("text-anchor", "end").text(text);
      let textWidth = estimateTextWidth(text);
      if (watermarkText.node()?.getComputedTextLength) {
        textWidth = watermarkText.node()?.getComputedTextLength();
      }
      const iconX = textRight - textWidth - iconGap - iconSize;
      selection.append("image").attr("transform", `translate(${iconX},${chartHeight + 25})`).attr("height", iconSize).attr("width", iconSize).attr("href", iconBase64);
    };
  }
});
...
var import_d3_scale, import_d3_selection, import_d3_shape, import_dayjs2, import_uniq2, margin, getDefaultOptions, getDarkThemeDefaultOptions, XYChart, xy_chart_default;
var init_xy_chart = __esm({
  "shared/packages/xy-chart.tsx"() {
    import_d3_scale = require("d3-scale");
    import_d3_selection = require("d3-selection");
    import_d3_shape = require("d3-shape");
    import_dayjs2 = __toESM(require("dayjs"), 1);
    import_uniq2 = __toESM(require("lodash/uniq"), 1);
    init_ToolTip();
    init_drawAxis();
    init_addFilter();
    init_addFont();
    ...
    XYChart = (svg, { title, xLabel, yLabel, data: { datasets }, showDots, theme, transparent }, initialOptions) => {
      const options = {
      ...
      const d3Selection = (0, import_d3_selection.select)(svg).style("stroke-width", 3).style("font-family", fontFamily).style("background", options.backgroundColor).attr("width", clientWidth).attr("height", clientHeight).attr("preserveAspectRatio", "xMidYMid meet");
      ...
      addFont_default(d3Selection);
      addFilter_default(d3Selection);
      if (options.envType === "browser") {
      ...
      const tooltip = new ToolTip_default({
      ...
      if (options.xTickLabelType === "Date") {
        data.datasets.forEach((dataset) => {
          dataset.data.forEach((d) => {
            d.x = (0, import_dayjs2.default)(d.x);
          });
        });
      }
      const allData = [];
      ...
      let xScale = (0, import_d3_scale.scaleTime)().domain([
        Math.min(...allXData.map((d) => Number(d))),
        Math.max(...allXData.map((d) => Number(d)))
      ]).range([0, chartWidth]);
      if (options.xTickLabelType === "Number") {
        xScale = (0, import_d3_scale.scaleLinear)().domain([0, Math.max(...allXData.map((d) => Number(d)))]).range([0, chartWidth]);
      }
      let yScale;
      if (options.useLogScale) {
        const maxYData = Math.max(...allYData);
        yScale = (0, import_d3_scale.scaleSymlog)().domain([0, maxYData]).range([chartHeight, 0]).constant(10);
      } else {
        yScale = (0, import_d3_scale.scaleLinear)().domain([0, Math.max(...allYData)]).range([chartHeight, 0]);
      }
      const svgChart = chart.append("g").attr("pointer-events", "all");
      drawWatermark(svgChart, chartWidth, chartHeight, options.watermarkText);
      if (title) {
        if ((0, import_uniq2.default)(datasets.map((d) => d.label.split("/")[0])).length === 1) {
      ...
      if (yLabel) {
        ...
        if (maxYData > 1e5) {
          offsetY = 2;
        } else if (maxYData > 1e4) {
          offsetY = 8;
        } else if (maxYData > 1e3) {
          offsetY = 12;
      ...
      if (options.showLine) {
        const drawLine = (0, import_d3_shape.line)().x((d) => xScale(d.x) || 0).y((d) => yScale(d.y) || 0).curve(import_d3_shape.curveMonotoneX);
        ...
        if (moltbotDataset && moltbotDataset.data.length > 0) {
          ...
          svgChart.append("text").attr("class", "moltbot-emoji browser-only").text("\u{1F99E}").attr("x", xScale(moltbotLastPoint.x) || 0).attr("y", (yScale(moltbotLastPoint.y) || 0) - 10).style("font-size", "20px").attr("text-anchor", "middle").attr("dominant-baseline", "auto");
          if (data.datasets.length >= 2) {
            const preyEmojis = ["\u{1F41F}", "\u{1F990}", "\u{1F980}", "\u{1F41A}", "\u{1F40C}", "\u{1FAB1}"];
            const usedEmojis = [];
            data.datasets.forEach((dataset) => {
      ...
      if (showDots) {
        const dotInitSize = 3.5 * (options.dotSize === void 0 ? 1 : options.dotSize);
        const dotHoverSize = 6 * (options.dotSize === void 0 ? 1 : options.dotSize);
        svgChart.selectAll(".xkcd-chart-xycircle-group").data(data.datasets).enter().append("g").attr("class", "xkcd-chart-xycircle-group").attr("filter", filter).attr("xy-group-index", (_, i) => i).selectAll(".xkcd-chart-xycircle-circle").data((dataset) => dataset.data).enter().append("circle").attr("class", "chart-tooltip-dot").style("stroke", (_, i, nodes) => {
          const xyGroupIndex = Number((0, import_d3_selection.select)(nodes[i].parentElement).attr("xy-group-index"));
          ...
          const xyGroupIndex = Number((0, import_d3_selection.select)(nodes[i].parentElement).attr("xy-group-index"));
          (0, import_d3_selection.select)(nodes[i]).attr("r", dotHoverSize);
          ...
          let title2 = (0, import_dayjs2.default)(data.datasets[xyGroupIndex].data[i].x).format(options.dateFormat);
          if (options.xTickLabelType === "Number") {
            const type = getTimestampFormatUnit(Number(data.datasets[xyGroupIndex].data[1].x || data.datasets[xyGroupIndex].data[i].x));
            title2 = getFormatTimeline_default(Number(data.datasets[xyGroupIndex].data[i].x), type);
          }
          tooltip.update({
            title: title2,
            items: [
          ...
          const i = [...nodes].indexOf(event.target);
          (0, import_d3_selection.select)(nodes[i]).attr("r", dotInitSize);
          tooltip.hide();
        });
      }
    ...
    xy_chart_default = XYChart;
  }
});

// server/svg-render.ts
var svg_render_exports = {};
__export(svg_render_exports, {
  renderSvg: () => renderSvg
});
function fixJsdomSvgCasing(svg) {
  return svg.replace(/feturbulence/g, "feTurbulence").replace(/fedisplacementmap/g, "feDisplacementMap").replace(/filterunits/g, "filterUnits").replace(/basefrequency/g, "baseFrequency").replace(/xchannelselector/g, "xChannelSelector").replace(/ychannelselector/g, "yChannelSelector");
}
function renderSvg(repoData, options) {
  const dom = new import_jsdom.JSDOM("<!DOCTYPE html><body></body>");
  ...
  return (0, import_svgo.optimize)(fixJsdomSvgCasing(svg.outerHTML), { multipass: true }).data;
}
var import_jsdom, import_svgo;
var init_svg_render = __esm({
  "server/svg-render.ts"() {
    import_jsdom = require("jsdom");
    import_svgo = require("svgo");
    init_xy_chart();
    init_chart();
  }
});

// server/svg-handler.ts
var svg_handler_exports = {};
__export(svg_handler_exports, {
  default: () => handler
});
module.exports = __toCommonJS(svg_handler_exports);

// server/github-config.ts
var REPO_PATTERN = /^[a-z0-9_.-]+\/[a-z0-9_.-]+$/i;
function parseAllowedRepos(value) {
  return (value || "").split(/[\s,]+/).map((repo) => repo.trim().toLowerCase()).filter(Boolean);
}
function isValidRepo(repo) {
  return REPO_PATTERN.test(repo);
}
function isRepoAllowed(repo, allowedRepos) {
  const normalizedRepo = repo.toLowerCase();
  const owner = normalizedRepo.split("/")[0];
  return allowedRepos.includes("*") || allowedRepos.includes(normalizedRepo) || allowedRepos.includes(`${owner}/*`);
}
...
function getRequestHost(req) {
  ...
  return getQueryValue(headers["x-forwarded-host"] || headers.host).split(",")[0].trim().replace(/[^a-z0-9.:[\]-]/gi, "") || "star-history.com";
}
...
async function handler(req, res) {
  ...
  const repos = getQueryValue(req.query.repos).split(",").map((repo) => repo.trim()).filter(Boolean);
  ...
  const type = getQueryValue(req.query.type).toLowerCase() === "timeline" ? "Timeline" : "Date";
  ...
  const useLogScale = req.query.logscale !== void 0 && getQueryValue(req.query.logscale) !== "false";
  const legendPosition = getQueryValue(req.query.legend) === "bottom-right" ? "bottom-right" : "top-left";
  const width = CHART_WIDTHS[getQueryValue(req.query.size)] || CHART_WIDTHS.laptop;
  try {
    const [{ getRepoData: getRepoData2 }, { renderSvg: renderSvg2 }] = await Promise.all([
      Promise.resolve().then(() => (init_chart(), chart_exports)),
      Promise.resolve().then(() => (init_svg_render(), svg_render_exports))
    ]);
    const repoData = await getRepoData2(repos, token, 15);
    await Promise.all(repoData.map(async (repo) => {
      repo.logoUrl = await toDataUrl(repo.logoUrl);
    }));
    const output = renderSvg2(repoData, {
      type,
      theme,
      transparent,
      useLogScale,
