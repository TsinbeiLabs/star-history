...
const estimateTextWidth = (text: string) => (Array.from(text).reduce((width, character) => {
...
export const drawWatermark = (selection: D3Selection, chartWidth: number, chartHeight: number, text = "star-history.com") => {
    ...
    const iconSize = 20
    const iconGap = 6
    const watermarkText = selection
        .append("text")
        .style("font-size", "16px")
        .style("fill", "#666666")
        .style("letter-spacing", "0.02em")
        .attr("transform", `translate(${textRight},${chartHeight + 40})`)
        .attr("text-anchor", "end")
        .text(text)

    let textWidth = estimateTextWidth(text)
    if (watermarkText.node()?.getComputedTextLength) {
        textWidth = watermarkText.node()?.getComputedTextLength() as number
    }
    const iconX = textRight - textWidth - iconGap - iconSize

    selection
        .append("image")
        .attr("transform", `translate(${iconX},${chartHeight + 25})`)
